...
# - Builds input text (prompt + evidence JSON + OUTPUT:)
# - Generates one-line multi-label analysis "<LABEL>: <reason>; ... [End]"
# - Returns a dict that can be directly consumed by GMS prompt builder.
# ------------------------------------------------------------
...
async def run_model2(model2_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    - Sets current_judgement.analysis to model2 analysis.
    - Keeps original payload fields for downstream GMS.
    """
    # avoid blocking event loop (GPU generate is heavy)
